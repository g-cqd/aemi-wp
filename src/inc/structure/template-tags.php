...
if ( ! function_exists( 'aemi_skip_content' ) ) {
	function aemi_skip_content() {
		?>
<a class="skip-link focusable screen-reader-text button" href="#<?php echo is_singular() ? 'post-content' : 'site-loop'; ?>" tabindex="0">
		<?php esc_html_e( 'Skip to content', 'aemi' ); ?>
</a>
		<?php
	}
}
...
if ( ! function_exists( 'aemi_body_classes' ) ) {
	function aemi_body_classes( $classes ) {
		...
		if ( in_array( $stickyness, array( 'top', 'adaptative' ), true ) ) {
...
if ( ! function_exists( 'aemi_html_tag_schema' ) ) {
...
if ( ! function_exists( 'aemi_featured_image' ) ) {
	function aemi_featured_image() {
		...
		$no_img            = 'no_img' === get_theme_mod( 'aemi_post_layout', 'cover' );
...
if ( ! function_exists( 'aemi_info_dates' ) ) {
	function aemi_info_dates() {
		...
		$pub_both   = 'both' === $pub_mod;
		$pub_none   = 'none' === $pub_mod;
		$pub_single = 'single' === $pub_mod;
		$pub_loop   = 'loop' === $pub_mod;

		$upt_mod    = get_theme_mod( aemi_type( 'updated_date' ), 'none' );
		$upt_both   = 'both' === $upt_mod;
		$upt_none   = 'none' === $upt_mod;
		$upt_single = 'single' === $upt_mod;
		$upt_loop   = 'loop' === $upt_mod;
...
if ( ! function_exists( 'aemi_info_author' ) ) {
	function aemi_info_author() {
		...
		$author_both   = 'both' === $author_mod;
		$author_none   = 'none' === $author_mod;
		$author_single = 'single' === $author_mod;
		$author_loop   = 'loop' === $author_mod;

		if (
			! $author_none &&
			(
				$author_both ||
				( $singular && $author_single || ! $singular && $author_loop )
			)
		) {
			$tag       = 'div';
...
if ( ! function_exists( 'aemi_sticky_badge' ) ) {
	function aemi_sticky_badge( $post_type ) {
		if ( 'post' === $post_type ) {
			...
			$sticky_both   = 'both' === $sticky_mod;
			$sticky_single = 'single' === $sticky_mod;
			$sticky_loop   = 'loop' === $sticky_mod;
			$sticky_none   = 'none' === $sticky_mod;
			if (
				$is_sticky &&
				! $sticky_none &&
				(
					$sticky_both ||
					( $singular && $sticky_single || ! $singular && $sticky_loop )
				)
			) {
				return true;
			}
	...
	function aemi_show_excerpt( $post_type ) {
		if ( ! is_singular() ) {
			if ( 'post' === $post_type ) {
				...
				$excerpt_both       = 'both' === $excerpt_mod;
				$excerpt_sticky     = 'sticky_only' === $excerpt_mod;
				$excerpt_non_sticky = 'non_sticky_only' === $excerpt_mod;
				$excerpt_none       = 'none' === $excerpt_mod;

				if (
					! $excerpt_none &&
					(
						$excerpt_both ||
						( $is_sticky && $excerpt_sticky || ! $is_sticky && $excerpt_non_sticky )
					)
				) {
					return true;
				}
			} elseif ( get_theme_mod( aemi_type( 'show_excerpt' ), 0 ) ) {
				return true;
			}
		}
		return false;
...
if ( ! function_exists( 'aemi_meta_header' ) ) {
	function aemi_meta_header() {
		...
		$categories = (
			...
			1 === get_theme_mod( aemi_type( 'category' ), 1 )
		);
...
<div class="post-meta">
			...
			if ( $sticky ) {
				?>
				<div class="meta-sticky"><?php echo esc_html__( 'Featured', 'aemi' ); ?></div>
				<?php
			}
			if ( $excerpt ) {
				?>
				<p class="meta-excerpt"><?php echo esc_html( get_the_excerpt() ); ?></p>
				<?php
			}
			if ( $categories ) {
				?>
...
if ( ! function_exists( 'aemi_get_post_meta' ) ) {
	function aemi_get_post_meta() {
		...
		foreach ( get_post_taxonomies() as $tax ) {
			if (
				1 === get_theme_mod( aemi_type( $tax ), 1 ) &&
				(
					'post_tag' === $tax ||
					( 'category' !== $tax && 'post_format' !== $tax )
				)
			) {
				$metas[] = $tax;
			}
	...
	function aemi_post_meta_footer() {
		...
		if ( is_singular() && count( $metas ) > 0 ) {
			...
			foreach ( $metas as $meta ) {
				if ( ! empty( get_the_term_list( $post->ID, $meta ) ) ) {
					$one_meta          = true;
					$meta_occ[ $meta ] = true;
				}
			}
	...
	<div class="meta-tags meta-item">
		...
		<div class="meta-item-list">
			<span class="meta-item-list-item button">
							...
							<?php
						} elseif ( 'category' !== $meta && 'post_format' !== $meta ) {
							?>
