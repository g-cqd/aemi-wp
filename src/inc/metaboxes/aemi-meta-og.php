	...
	function aemi_add_meta_og() {
		foreach ( get_post_types( array( 'public' => true ) ) as $type ) {
			...
			if ( ! wp_script_is( 'aemi-meta-og', 'enqueue' ) ) {
				wp_enqueue_script(
					'aemi-meta-og',
					trailingslashit( get_template_directory_uri() ) . 'inc/metaboxes/js/aemi-meta-og.js',
					array( 'jquery' )
				);
			}
		}
...
if ( ! function_exists( 'aemi_get_meta_data' ) ) {
	...
	function aemi_get_meta_data( $post ) {
		...
		$data = array(
			'site_name' => array( 'value' => get_bloginfo( 'name' ) ),
			'url'       => array( 'value' => esc_url( home_url( $wp->request ) ) ),
		);

		if ( isset( $wp_post ) ) {
			$data['author']      = array( 'value' => get_post_meta( $wp_post->ID, 'aemi_meta_author', true ) );
			$data['description'] = array( 'value' => get_post_meta( $wp_post->ID, 'aemi_meta_description', true ) );
			$data['keywords']    = array( 'value' => get_post_meta( $wp_post->ID, 'aemi_meta_keywords', true ) );
			$data['title']       = array( 'value' => get_post_meta( $wp_post->ID, 'aemi_meta_og_title', true ) );
			$data['image']       = array( 'value' => get_post_meta( $wp_post->ID, 'aemi_meta_og_image', true ) );
			$data['type']        = array( 'value' => get_post_meta( $wp_post->ID, 'aemi_meta_og_type', true ) );
		} else {
			$data['author']      = array( 'value' => '' );
			$data['description'] = array( 'value' => '' );
			$data['keywords']    = array( 'value' => '' );
			$data['title']       = array( 'value' => '' );
			$data['image']       = array( 'value' => '' );
		...
		if ( empty( $data['image']['value'] ) ) {
			if ( isset( $wp_post ) && has_post_thumbnail( $wp_post ) ) {
				...
				if ( ! empty( $dark_scheme_logo ) ) {
					$data['image']['value'] = $dark_scheme_logo;
				} elseif ( ! empty( $light_scheme_logo ) ) {
	...
	function aemi_meta_og_callback( $post ) {
		...
		<fieldset>
			<label for="aemi_meta_og_title">
				<?php esc_html_e( 'Open Graph: Title', 'aemi' ); ?>
		...
		<fieldset>
			<label for="aemi_meta_og_description">
				<?php esc_html_e( 'Open Graph: Description', 'aemi' ); ?>
		...
		<fieldset>
			<label for="aemi_meta_og_image">
				<?php esc_html_e( 'Open Graph: Image', 'aemi' ); ?>
				<button class="button aemi_meta_og_media_button" data-custom-plugin-media-uploader-target=".aemi_meta_og_image"><?php esc_html_e( 'Upload File', 'aemi' ); ?></button>
		...
		<fieldset>
			<label for="aemi_meta_og_type">
				<?php esc_html_e( 'Open Graph: Type', 'aemi' ); ?>
				<select id="aemi_meta_og_type" name="aemi_meta_og_type">
					<?php foreach ( $types as $val ) { ?>
					<option value="<?php echo esc_attr( $val ); ?>" <?php echo selected( $val, $opt['type']['value'] ); ?>>
...
if ( ! function_exists( 'aemi_meta_og__action' ) ) {
	function aemi_meta_og__action() {
		...
		if ( '' !== $data['site_name']['value'] ) {
			?>
			<meta property="og:site_name" content="<?php echo esc_attr( $data['site_name']['value'] ); ?>">
			<?php
}
		if ( '' !== $data['url']['value'] ) {
			?>
			<meta property="og:url" content="<?php echo esc_attr( $data['url']['value'] ); ?>">
			<?php
}
		if ( '' !== $data['title']['value'] ) {
			?>
			<meta property="og:title" content="<?php echo esc_attr( $data['title']['value'] ); ?>">
			<?php
}
		if ( '' !== $data['description']['value'] ) {
			?>
			<meta property="og:description" content="<?php echo esc_attr( $data['description']['value'] ); ?>">
			<?php
}
		if ( '' !== $data['image']['value'] ) {
			?>
			<meta property="og:image" content="<?php echo esc_attr( $data['image']['value'] ); ?>">
			<?php
}
		if ( '' !== $data['type']['value'] ) {
			?>
			<meta property="og:type" content="<?php echo esc_attr( $data['type']['value'] ); ?>">
			<?php
}
	}
}

if ( 1 === get_theme_mod( 'aemi_add_meta_og', 0 ) ) {
