	...
	function aemi_add_meta_twitter() {
		foreach ( get_post_types( array( 'public' => true ) ) as $type ) {
			...
			if ( ! wp_script_is( 'aemi-meta-twitter', 'enqueue' ) ) {
				wp_enqueue_script(
					'aemi-meta-twitter',
					trailingslashit( get_template_directory_uri() ) . 'inc/metaboxes/js/aemi-meta-twitter.js',
					array( 'jquery' )
				);
			}
		}
...
if ( ! function_exists( 'aemi_meta_twitter_data_fallback' ) ) {
	 ...
	 * @param type $type Current post type.
	 * @return mixed
	 */
	...
	function aemi_meta_twitter_data( $post ) {
		...
		if ( ! is_enabled( 'aemi_add_meta_og', 0 ) ) {
			$data = array_filter(
				...
				function ( $key ) {return in_array( $key, array( 'title', 'description', 'image' ) );},
				ARRAY_FILTER_USE_KEY
			);
		}

		$data['card']    = array( 'value' => aemi_meta_twitter_data_fallback( $post, 'card' ) );
		$data['site']    = array( 'value' => aemi_meta_twitter_data_fallback( $post, 'site' ) );
		$data['creator'] = array( 'value' => aemi_meta_twitter_data_fallback( $post, 'creator' ) );
	...
	function aemi_meta_twitter_callback( $post ) {
		...
		<fieldset>
			<label for="aemi_meta_twitter_card">
				<?php esc_html_e( 'Twitter: Card Type', 'aemi' ); ?>
		...
		<fieldset>
			<label for="aemi_meta_twitter_site">
				<?php esc_html_e( 'Twitter: Site Account', 'aemi' ); ?>
		...
		<fieldset>
			<label for="aemi_meta_twitter_creator">
				<?php esc_html_e( 'Twitter: Creator Account', 'aemi' ); ?>
		...
		<fieldset>
			<label for="aemi_meta_twitter_title">
				<?php esc_html_e( 'Twitter: Content Title', 'aemi' ); ?>
		...
		<fieldset>
			<label for="aemi_meta_twitter_description">
				<?php esc_html_e( 'Twitter: Content Description', 'aemi' ); ?>
		...
		<fieldset>
			<label for="aemi_meta_twitter_image">
				<?php esc_html_e( 'Twitter: Content Image', 'aemi' ); ?>

				<button class="button aemi_meta_twitter_media_button" data-custom-plugin-media-uploader-target=".aemi_meta_twitter_image"><?php esc_html_e( 'Upload File', 'aemi' ); ?></button>
...
if ( ! function_exists( 'aemi_save_meta_twitter_data' ) ) {
	/**
	 * @param post_id $post_id Id of the post.
	 * @return mixed
	 */
...
if ( ! function_exists( 'aemi_meta_twitter__action' ) ) {
	function aemi_meta_twitter__action() {
		...
		if ( '' !== $data['card']['value'] ) {
			?>
			<meta property="twitter:card" content="<?php echo esc_attr( $data['card']['value'] ); ?>">
			<?php
		}
		if ( '' !== $data['site']['value'] ) {
			?>
			<meta property="twitter:site" content="<?php echo esc_attr( $data['site']['value'] ); ?>">
			<?php
		}
		if ( '' !== $data['creator']['value'] ) {
			?>
			<meta property="twitter:creator" content="<?php echo esc_attr( $data['creator']['value'] ); ?>">
			<?php
		}
		if ( ! is_enabled( 'aemi_add_meta_og', 0 ) ) {
			if ( '' !== $data['title']['value'] ) {
				?>
				<meta property="twitter:title" content="<?php echo esc_attr( $data['title']['value'] ); ?>">
				<?php
			}
			if ( '' !== $data['description']['value'] ) {
				?>
				<meta property="twitter:description" content="<?php echo esc_attr( $data['description']['value'] ); ?>">
				<?php
			}
			if ( '' !== $data['image']['value'] ) {
				?>
				<meta property="twitter:image" content="<?php echo esc_attr( $data['image']['value'] ); ?>">
				<?php
			}
		}
	}
