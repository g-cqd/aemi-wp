...
if ( ! function_exists( 'aemi_post_navigation' ) )
{
	function aemi_post_navigation()
	{
		?><nav id="post-navigation" class="pagination" role="navigation"><?php
			previous_post_link('<div class="nav-previous">%link</div>', '<span class="nav-arrow">&larr; ' . __( 'older', 'aemi' ) . '</span><span class="nav-title">%title</span>');
			next_post_link('<div class="nav-next">%link</div>', '<span class="nav-arrow">' . __( 'newer', 'aemi' ) . ' &rarr;</span><span class="nav-title">%title</span>');
		?></nav><?php
	}
}
